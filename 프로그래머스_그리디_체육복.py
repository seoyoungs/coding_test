...
체육복을 도난당한 학생의 수는 1명 이상 n명 이하이고 중복되는 
번호는 없습니다.
...
예제 #2
3번 학생이 2번 학생이나 4번 학생에게 체육복을 빌려주면 학생 4명이 
체육수업을 들을 수 있습니다.
'''
def solution(n, lost, reserve):
    # 체육복 잃어버린 사람 lost
    # reserve 여벌의 체육복을 가져온 학생
    common_1 = set(lost) - set(reserve) # lost, reserve 중복요소 제거
    common_2 = set(reserve) - set(lost)
    for i in common_2:
        if i-1 in common_1: # set_reserve를 순회하며 양옆번호에 잃버린 학생이 있다면 왼쪽학생을 우선으로 빌려주는 방법을 사용하였다.
            common_1.remove(i-1)
        elif i+1 in common_1:
            common_1.remove(i+1)
    return n-len(common_1) # 전체학생수 - 체육복갯수가 0개인 학생의 수를 반환하여 해결 하였다.
